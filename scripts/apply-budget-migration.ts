/**
 * Apply Budget Tables Migration
 * This script creates the necessary database tables for the Budget & Usage feature
 */
...
async function applyMigration() {
    ...
    `);
        ...
        // Create indexes
        await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_user_budgets_user_id ON user_budgets(user_id)`);
        await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_budget_usage_history_user_id ON budget_usage_history(user_id)`);
        await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_budget_usage_history_period ON budget_usage_history(period_start, period_end)`);
        await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_budget_alerts_user_id ON budget_alerts(user_id)`);
        await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_budget_alerts_is_read ON budget_alerts(is_read)`);
