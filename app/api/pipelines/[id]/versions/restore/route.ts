/**
 * Pipeline Version Restore API
 *
 * Phase 8: Versioning & Deployment Lifecycle
 *
 * POST /api/pipelines/[id]/versions/restore
 * Restores a previous version to the draft (editor)
 */
...
const logger = createLogger('api:pipelines:versions:restore');
...
export async function POST(
  ...
  try {
    ...
    // Get version info before restore
    ...
    // Restore
    await WorkflowVersionService.restoreDraft({
      workflowId,
      versionId,
      userId: user.id,
    });

    logger.info('Version restored to draft', {
      workflowId,
      versionId,
